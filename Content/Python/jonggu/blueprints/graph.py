...
from jonggu.assets import asset_path as resolve_asset_path
...
def ensure_bp(name, parent):
    ...
    package_folder = asset_path.rsplit('/', 1)[0]
    bp = ASSETS.load_asset(asset_path) if ASSETS.does_asset_exist(asset_path) else None
    if bp is None:
        factory = unreal.BlueprintFactory()
        factory.set_editor_property('parent_class', parent)
        bp = unreal.AssetToolsHelpers.get_asset_tools().create_asset(name, package_folder, unreal.Blueprint, factory)
...
class G:
    ...
    def cast(graph, obj, cls, pure=True):
        ...
        defaults = unreal.get_default_object(target)
        base = next((native for native in (unreal.SaveGame, unreal.GameInstance, unreal.HUD, unreal.Actor)
                     if isinstance(defaults, native)), unreal.Object)
