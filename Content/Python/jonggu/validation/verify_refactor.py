...
def validate_refactor():
    ...
    try:
        ...
        fixture_meta = json.loads((ROOT / "Saved/RefactorQA/legacy_save.json").read_text(encoding="utf-8-sig"))
        ...
        check("legacy fixture records the actual pre-rename class",
              fixture_meta.get("old_class_path") in {old + "." + old.rsplit("/", 1)[1] + "_C"
                  for old, new in LEGACY_ASSET_PATHS.items() if new == asset_path("BP_RestaurantSaveGame")})
        fixture_file = save_dir / (FIXTURE_SLOT + ".sav")
        before_hash = hashlib.sha256(fixture_file.read_bytes()).hexdigest()
        ...
        check("legacy SaveGame resolves to canonical renamed class",
              loaded.get_class().get_path_name() == generated_class_path("BP_RestaurantSaveGame"),
        ...
        before = json.loads((ROOT / "Saved/RefactorQA/actors_before.json").read_text(encoding="utf-8-sig"))
        ...
        for map_name, scene in layout["maps"].items():
            ...
            source = {next(tag for tag in item["tags"] if tag.startswith("JongguMigration:")): item
                      for item in before[scene["package"].rsplit("/", 1)[1]]
