...
이 파일은 Codex가 매 세션 처음 받는 짧은 온보딩 맵이다. 모델은 세션 간 프로젝트 상태를 기억하지 않으므로, 여기에는 항상 유효한 WHAT/WHY/HOW만 둔다. 작업 하네스는 [Docs/README.md](Docs/README.md)가 맡고, 세부 규칙은 하위 정본 문서로 점진 공개한다.
