...
- `HUDRoot` 또는 `PopupRoot` 하위 이름을 바꾸면 `Assets/Scripts/UI/UIManager.cs`, `Assets/Scripts/UI/Layout/PrototypeUISceneLayoutCatalog.cs`, 관련 UI 문서를 함께 갱신합니다. 팝업 타이틀/캡션 공용 이름이면 `PrototypeUIObjectNames.cs`도 함께 맞춥니다.
