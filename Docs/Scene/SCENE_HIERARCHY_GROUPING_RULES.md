...
- 런타임/에디터 정리 도구는 `scene-hierarchy-contracts.asset`에 저장된 parent/sibling/initialActive baseline override를 catalog 기본값보다 먼저 적용합니다.
...
- Canvas 루트 변경: [UI_GROUPING_RULES.md](../UI/UI_GROUPING_RULES.md), `UIManager`, `PrototypeUISceneLayoutCatalog`, `PrototypeUILayout*`
- 허브/월드 구조 변경: 관련 `Exploration/World` 코드와 씬 직렬화
- 씬에 저장되는 `authored helper object` 변경: 부모, 이름, 초기 active baseline, 참조가 scene contract로 유지되는지 함께 확인합니다. 새 helper를 authored 정본으로 취급하려면 `SceneAuthoredHelperContractMarker`를 붙입니다.
...
- 플레이 중 active 상태 변화는 runtime state가 소유하고, organizer의 `initialActive` 적용은 에디터 구조 복원 기준인지 함께 확인합니다.
