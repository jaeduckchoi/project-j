...
허브는 영업, 정리, 성장, 탐험 출발이 모두 이뤄지는 공간이다.
플레이어는 허브를 직접 걸어 다니며 메뉴를 고르고, 창고를 정리하고, 업그레이드를 진행하고, 외부 출구로 `Beach`(바닷가)에 나간다.
세부 탐험 지역 결정은 바닷가를 중심 연결 노드로 삼아 이뤄진다.
...
- 외부 출구 (`Beach` 진입점)
- 바닷가 이후 지역 연결 (`Sea`, `DeepForest`, `Shortcut`, `WindHill`, `AbandonedMine`)
