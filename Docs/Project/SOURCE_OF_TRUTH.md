...
| `authored helper object` | 저장 의도를 가진 씬 오브젝트 자체 | 관련 씬 직렬화 값, 동기화 컴포넌트, [GAME_SCENE_AND_SETUP.md](../Scene/GAME_SCENE_AND_SETUP.md) | 임시 프리뷰 오브젝트와 혼동하지 않는다. |
...
| Canvas 관리 UI | `PrototypeUISceneLayoutCatalog.cs`, `PrototypeUILayout*.cs`, `Assets/Resources/Generated/ui-layout-bindings.asset` | `UIManager.cs`, `UIManager.*.cs`, `PrototypeUIDesignController.cs`, [UI_AND_TEXT_GUIDE.md](../UI/UI_AND_TEXT_GUIDE.md), [UI_GROUPING_RULES.md](../UI/UI_GROUPING_RULES.md) | override가 없으면 씬에 이미 저장된 managed UI 값을 유지한다. |
