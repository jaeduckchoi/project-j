...
- `Assets/Level/Scenes` 아래 실제 씬 직렬화 값이 월드 구조의 정본이다.
...
- 최종 허브 배치 정본은 `Assets/Level/Scenes/Hub.unity` 직렬화 값이다.
...
- 레이아웃 편집기에서 선택한 씬 UI 오브젝트를 캡처하면 `ui-layout-bindings.asset`에 즉시 저장되고, 저장된 binding 또는 같은 이름으로 유일하게 resolve되는 managed UI 오브젝트는 씬 저장 시 asset으로 다시 동기화된다.
...
- authored CSV 원본은 `Assets/Data/GameDataSource/*.csv`다.
- CSV import와 generated 출력 생성의 정본은 `Assets/Code/Editor/GameData/CsvGameDataImporter.cs`다.
- CSV 원본을 바꾸면 generated GameData와 fallback 동작을 함께 확인한다.
...
- authored 스프라이트 원본은 도입 시 `Assets/Art/*`를 사용한다.
- import 정책 정본은 `Assets/Code/Editor/Art/ArtSpriteImportPostprocessor.cs`다.
...
- CSV 원본/생성 경로 변경: `Assets/Data/GameDataSource/*.csv`, `CsvGameDataImporter.cs`, `PrototypeGeneratedAssetSettings.cs`, `Docs/build/GAME_BUILD_GUIDE.md`
