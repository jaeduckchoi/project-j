...
SESSION_BP=unreal.load_asset('/Game/Jonggu/Prototype/BP_PrototypeSession')
...
def current():
    w=getworld()
    m=unreal.GameplayStatics.get_actor_of_class(w,unreal.load_class(None,'/Game/Jonggu/Prototype/BP_PrototypeManager.BP_PrototypeManager_C')) if w else None
    return w,m
...
def tick(dt):
    ...
    try:
        ...
        if s=='ready':
            ...
            check('dedicated controller is active',unreal.GameplayStatics.get_player_controller(w,0).get_class().get_name()=='BP_PrototypeController_C')
            ...
            from verify_gameplay import validate_gameplay
            REPORT['gameplay']=validate_gameplay(m)
            from verify_popups import validate_popups
            REPORT['popups']=validate_popups(m)
            from verify_experiments import validate_experiments
