...
from verify_popups import PopupFixture, button_center, validate_shared_geometry
...
SESSION_CDO = unreal.get_default_object(unreal.load_asset("/Game/Jonggu/Prototype/BP_PrototypeSession").generated_class())
...
if HUD_MODE:
    from verify_main_hud import HUD_CASES, stage_main_hud, main_hud_metadata, validate_main_hud_runtime
    CASES = HUD_CASES
if ADAPTIVE_MODE:
    from verify_adaptive_hud import (ADAPTIVE_CASES, stage_adaptive_hud, adaptive_metadata,
        validate_adaptive_runtime, sample_frame, click_faded_inventory)
    CASES = ADAPTIVE_CASES
if COVERAGE_MODE:
    from verify_adaptive_hud import COVERAGE_CASES, stage_coverage_hud, coverage_metadata
...
def current():
    ...
    manager = unreal.GameplayStatics.get_actor_of_class(world, unreal.load_class(None,
        "/Game/Jonggu/Prototype/BP_PrototypeManager.BP_PrototypeManager_C")) if world else None
