...
`.aiassistant`는 이 저장소의 작업 하네스다. 루트 엔트리 파일은 매 세션 자동 주입되는 짧은 온보딩 맵이고, 실제 규칙과 정본 관계는 **프로젝트 최상위 `Docs` 디렉터리** 아래 문서에 기록한다.

## 0. 엔트리 파일 원칙

- `AGENTS.md`와 `CLAUDE.md`는 상태를 기억하지 않는 에이전트에게 프로젝트의 WHAT/WHY/HOW를 짧게 알려주는 문서다.
- 루트 엔트리에는 모든 세션에 보편적으로 필요한 정보만 둔다.
- 세부 지침은 이 허브와 `Docs/*` 정본 문서로 연결해 점진 공개한다.
- 새 규칙은 루트 엔트리에 바로 추가하지 말고, 먼저 정본 문서 위치를 정한 뒤 링크만 연결한다.
- 루트 엔트리는 자동 생성하지 않고 수작업으로 유지한다.
...
- 엔트리 파일에는 작업별 예외, 긴 명령어 목록, 스타일 규칙을 넣지 않는다.
